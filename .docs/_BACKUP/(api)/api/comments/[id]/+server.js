...
export async function PATCH({ request, params, locals: { supabase, getSession } }) {
	...
	try {
		...
		// For PATCH requests, we typically don't cache the response
		// but we can set no-cache headers
		const response = json({ message: 'Comment updated successfully!', comment: updatedComment }, { status: 200 });
...
export async function DELETE({ params, locals: { supabase, getSession } }) {
	...
	// For DELETE requests, we typically don't cache the response
	// but we can set no-cache headers
	const response = json({ message: 'Comment deleted successfully!' }, { status: 200 });
	response.headers.set('Cache-Control', 'no-cache, no-store, must-revalidate');
	response.headers.set('Pragma', 'no-cache');
	response.headers.set('Expires', '0');
