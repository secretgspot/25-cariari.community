...
export async function PATCH({ request, params, locals: { supabase, getSession } }) {
	...
	try {
		...
		const response = json({ message: 'Notice updated successfully!', notice: updatedNotice }, { status: 200 });
...
export async function DELETE({ params, request, locals: { supabase, getSession } }) {
	...
	try {
		...
		const response = json({ message: 'Notice deleted successfully!' }, { status: 200 });
		response.headers.set('Cache-Control', 'no-cache, no-store, must-revalidate');
		response.headers.set('Pragma', 'no-cache');
		response.headers.set('Expires', '0');
