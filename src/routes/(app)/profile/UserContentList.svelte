<script>
  ...
  let { title, items, type, itemKey, dateKey = 'created_at', linkPrefix, additionalInfo, deleteHandler } = $props();

  function getCommentLink(item) {
    if (item.event_id) return `/events/${item.event_id}`;
    if (item.news_id) return `/notices/${item.news_id}`;
  ...
  {#if items && items.length > 0}
    <ul>
      {#each items as item (item.id)}
        <li class="list-item">
          <a href={type === 'comment' ? getCommentLink(item) : `${linkPrefix}/${item.id}`} class="item-content">
            <div class="main-info">
